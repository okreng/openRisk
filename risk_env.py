...
class RiskEnv():
	...
	def player_name_to_agent(self, player_name, player_id):
		...
		elif player_name == "deep":
			return agent.Agent(player_id, self.game.graph.total_territories, self.game.graph.edge_list, "amass", "deep_attack_net", "skip_fortify", self.verbose)
...
def imitation_learn(board, matchup, verbose, print_game, train=False, num_games=100, num_epochs=100000):
	...
	MODEL_INSTANCE = '0-7'
	LEARNING_RATE = 1e-10
	TRIM_TO_STATES = 10000 #216 #318 ## max number of states  # 318  ## mean - standard deviation
	...
	if train:
		...
		# training_policy = three_layer_attack_net.ThreeLayerAttackNet(environment.game.graph.total_territories, environment.game.graph.edge_list, MODEL_INSTANCE, 0, LEARNING_RATE)
		...
		# from q_funcs.attack import deep_attack_net
		# training_policy = deep_attack_net.DeepAttackNet(environment.game.graph.total_territories, environment.game.graph.edge_list, MODEL_INSTANCE, 0, LEARNING_RATE)
		...
		while epoch < num_epochs:
			...
			if (epoch%(USEFUL_LIFE/100)) == 0:
				...
				accuracy_std = np.std(v_accuracy)
				plt.subplot(2,1,2)
	...
	for game in range(num_games):
		...
		for player in player_list:
			...
			if player == winner and (batch_size[player] != 0):
				batch_loss, _ = training_policy.batch_train(state_vector=reinforcement_states[player], action_vector=reinforcement_targets[player], valid_mask=reinforcement_masks[player], update=True, batch_size=batch_size[player])
				train_loss.append(np.mean(batch_loss))
